...
"""
    ...
    def dump(self, filename: str, size=0x8000) -> None:
...
if __name__ == '__main__':
    ...
    dump_cmd.add_argument('-f', '--file', help='specify the file to dump to',
                          type=str, required=True, default="")
    dump_cmd.add_argument('-s', '--size', help='only dump the first n bytes',
                          type=int, required=False, default=0x8000)
    load_cmd = cmds.add_parser('load', help='loads up to 32kb of stdin onto the EEPROM')
    load_cmd.add_argument('-f', '--file', help='specify the file to load',
                          type=str, required=True, default="")
    ...
    try:
        if args.cmd == 'dump':
            ...
            #    eeprom.fdump(f, size=args.size, console=None)
            eeprom.dump(args.file, args.size)
        elif args.cmd == 'load':
